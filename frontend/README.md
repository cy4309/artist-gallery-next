...
## About「請我們喝杯咖啡」（收款，暫未上線）

About 頁 `#coffee` 區塊目前**註解隱藏**。若要接上 NT$50 個人支持收款，**建議優先使用藍新金流（NewebPay）快速收款連結（EPG）**：個人可申請、免開戶費、固定金額連結可長期掛在網站上。

### 接上步驟（藍新）

1. 至 [藍新個人會員註冊](https://www.newebpay.com/website/Page/content/register) 開通並完成審核  
2. 後台建立商店 → 申請信用卡（或 ATM／超商）→ [快速收款連結](https://www.newebpay.com/website/Page/content/service_paymenttools) 建立 NT$50 商品頁  
3. 複製付款連結，寫入 env：`NEXT_PUBLIC_COFFEE_PAYMENT_URL=https://...`  
4. 取消註解 `app/about/page.tsx` 內 `#coffee` 區塊與 `coffeePaymentUrl` 常數  
5. Vercel Production 同步設定 env 後重新部署  

i18n 文案已備於 `src/locales/zh.ts`、`en.ts` 的 `about.coffee.*`。

### 方案比較（2026，個人 + 網站 NT$50 支持）

| 項目 | 藍新 NewebPay | 綠界 ECPay | LINE Pay（合作商店） | LINE Pay Money |
|------|---------------|------------|----------------------|----------------|
| **個人能否申請** | ✅ 個人會員 | ✅ 個人賣家 | ❌ 網路店面個人不可（僅實體／餐飲網路例外） | ✅ 個人電支帳戶 |
| **開戶／月費** | 免費 | 免費 | 免費 | 免費 |
| **網站付款連結** | ✅ EPG 快速收款連結 | ⚠️ 需串接或 ECShop；個人較繁 | ✅ 特店網站／連結（需審核） | ❌ 無永久連結 |
| **QR 放網站** | ✅ 連結或 QR 圖（EPG） | ⚠️ 同上 | ✅ 固定式商家 QR（需特店） | ❌ TWQR 約 **1 天**有效，需每日重產 |
| **固定 NT$50** | ✅ | ✅ | ✅（特店） | ⚠️ 對方手動輸入金額 |
| **手續費（參考）** | 信用卡約 2.8%／筆 | 信用卡约 2.75%／筆 | 约 3%／筆 | 用戶互轉免費 |
| **審核難度（個人）** | 低 | 中（環境照、網站聯絡資訊須一致） | 高（網路需統編或可購買官網） | 無（但無法當常駐收款） |
| **本專案建議** | **⭐ 優先** | 備選 | 有統編再評估 | 不適合 About 常駐收款 |

> LINE Pay Money 的「跨機構轉帳 → 顯示 TWQR」為動態收款碼，不適合截圖長期放在 About 頁。
